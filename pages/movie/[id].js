                ...
                <section className="relative">
                    ...
                    <div className="absolute inset-y-28 md:inset-y-auto md:bottom-10 inset-x-4 md:inset-x-12 space-y-6 z-50">
                        ...
                        <div className="flex space-x-3 md:space-x-5 items-center">
                            ...
                            <button className="text-xs md:text-base bg-black/30 text-[#f9f9f9] border border-white flex items-center justify-center py-2.5 px-6 rounded hover:bg-[#c6c6c6]" onClick={() => setShowPlayer(true)}>
                            ...
                            <div className="rounded-full border-2 border-white flex items-center justify-center w-11 h-11 cursor-pointer bg-black/60">
                                <img src="/images/group-icon.svg" className="h-6"/>
                            </div>
                        </div>
                        <p className="text-xs md:text-sm">
                            {result.release_date || result.first_air_date}{" "}{Math.floor(result.runtime/60)}h {result.runtime % 60}m{" "}{result.genres.map((genre)=> genre.name + " ")}{" "}
                        </p>
                        <h4 className="text-sm md:text-lg">{result.overview}</h4>
                    </div>
                    ...
                    <div className={`absolute top-3 inset-x-[7%] md:inset-x-[13%] rounded overflow-hidden transition duration-1000 ${showPlayer ? "opacity-100 z-50" : "opacity-0"}`}>
                        <div className="flex items-center justify-between bg-black text-[3f9f9f9] p-3.5">
                            <span className="font-semibold">Playing Trailer</span>
                            <div className="cursor-pointer w-8 h-8 flex justify-center items-center rounded-lg opacity-50 hover:opacity-75 bg-[#0f0f0f]" onClick={()=> setShowPlayer(false)}>
                                <XIcon className="h-5"/>
                            </div>
                        </div>
...
export async function getServerSideProps(context){
    ...
    const request = await fetch(`https://api.themoviedb.org/3/movie/${id}?api_key=${process.env.API_KEY}&language=en-US&append_to_response=videos`).then((response) =>
        response.json()
